...
let to_str pkt =
  let rec aux pkt str =  
    match pkt with
      ...
      | IP (h, p)   -> 
        let s = sprintf "%s|IP(%s)" str (Ip.to_str h) in aux p s
      | TCP (h, p)  -> 
        let s = sprintf "%s|TCP(%s)" str (Tcp.to_str h) in aux p s
      | UDP (h, p)  -> 
        let s = sprintf "%s|UDP(%s)" str (Udp.to_str h) in aux p s
...
let to_string pkt =
  let rec aux pkt str =  
    match pkt with
      ...
      | IP (h, p)   -> 
        let s = sprintf "%s|IP(%s)" str (Ip.to_string h) in aux p s
      | TCP (h, p)  -> 
        let s = sprintf "%s|TCP(%s)" str (Tcp.to_string h) in aux p s
      | UDP (h, p)  -> 
        let s = sprintf "%s|UDP(%s)" str (Udp.to_string h) in aux p s
