...
let pcap_of_filename filename =
  ...
  let open Ocap in
  let (fh, packets) = Pcap.iter buf (Demux.(eth_demux () ethertype_demux)) in
  { filename; filesize }, fh, packets
...
let erf_of_filename filename =
  ...
  let open Ocap in
  let packets = Erf.iter buf Demux.(eth_demux () ethertype_demux) in
  { filename; filesize }, packets
